...
if collect_data:
    ...
    folder = parent_folder +'/tests_simulator_behaviour/'+ '02_test'
...
if __name__ == '__main__':
    ...
    median_err = np.inf
    tasks['visual_servoing'] = {'visual_servoing':True, 'corner_detection':False, 'filter':filter, 'next_task_condition':{'median_error':median_err, 'error_array':errors}}
    ...
    while robot.step(timestep) != -1:
        ...
        if collect_data:
            if it_idx % sampling_frequency == 0: # Save one image per second
                # Save image

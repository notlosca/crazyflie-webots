## NO MORE USED. SEE IBVS_GT_NO_PHYS
...
dataset_path = '../../datasets/EXP-2-FRAME-IBVS-DELTA'
...
while robot.step(timestep) != -1 and err > thresh: # and frame_n < n_samples: # with this condition the controller will exit
    ...
    T_C = SE3(wd_tr)*SE3.RPY(roll,pitch,yaw)*SE3(dc_tr)*SE3.RPY(-np.pi/2, 0, -np.pi/2) # Relative pose in the camera frame
    ...
    # T_C = np.array(T_C)@np.array(SE3.Delta(v_camera)) 
    # T_D = T_C @ extrinsic_matrix_drone_camera.T # Camera position in drone-frame
    # T_W = T_C @ extrinsic_matrix.T # Camera position in world-frame
    # nextX, nextY, nextZ = T_W[:3,-1]
    # rot_drone = T_C[:3,:3] @ rotation_matrix_drone_camera.T
    # rot_ax = transforms3d.axangles.mat2axangle(rot_drone)
    # ax_angle = [rot_ax[0][0], rot_ax[0][1], rot_ax[0][2]] # axis
    # ax_angle.append(rot_ax[-1]) # append the angle
    # 
    # ########### ------------------ NO INTEGRATION ------------------ ###########
    ...
    if collect_data:
        ...
        sample['projected_points'] = p_detected
    ...
    #     x, y = pd[:,id]
    #     plt.scatter(x, y, marker='*', c=col, s=50)
    #     x, y = p_detected[:,id]
    #     plt.scatter(x, y, marker='x', c=col, s=50)
    #
